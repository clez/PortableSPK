<?php
/**
 * Portable SPK Source - full example including class
 *
 * useage:
 *	 name it "index.php" and drop it in your package webfolder
 * 
...
$PortableSPK->set(array(
//	'public_url'		=>	'http://packages.example.com/packages/',
//	'language'		=>	isset($_REQUEST['language']) ? $_REQUEST['language'] : 'enu',
	'debug'			=>	isset($_GET['debug']),		// set "false" to disable debugging
//	'cache_enabled'		=>	false,				// set "true" when cache_path is ready
//	'cache_path'		=>	'/tmp/portablespk.cache'	// NOT within download directory (filemtime loop), within open_basedir()
));
...
function nice_size($return,$ibi = false) {
	$i = 0;
	while($return > 900) {
		$return /= ($ibi ? 1000 : 1024);
		$i++;
	}
	$m = array('B','KB','MB','GB','TB','PB');
	return str_replace('.0','',sprintf('%0.1f',$return)).' '.$m[$i];
}
/* }}} */
...
<head>
  ...
  <title>%s</title>
  <style type="text/css">
body 		{font-family:Verdana,Tahoma,Arial,Sans-Serif; width:800px}
table,input,div	{padding:20px; margin:20px }
table		{width:800px;border-bottom:1px solid #f90}
pre span	{font-weight:bold; color:#f00 }
th		{text-align:left}
td.i		{width:100px;text-align:center}
td.r		{width:300px;text-align:right}
.sum		{font-family:courier,system,fixed; font-size:80%%}
input		{border:0;border-bottom:1px solid #00f; width:450px; padding:4px 25px;font-weight:bold}
  </style></head>
<body>
  <div><h1>%s</h1>
	<p>Package source: <input type="text" readonly="readonly" value="%s" /></p>',
	strip_tags($PortableSPK->public_name), $PortableSPK->public_name, $PortableSPK->public_url);
if(!$PortableSPK->cache_enabled) {
	echo('<p>Warning: SPK INFO caching disabled.</p>');
}
foreach($PortableSPK as $file=>$p) {				// iterate $PortableSPK for PHP arrays of packages
	$p['desc'] = isset($p['desc_'.$PortableSPK->language]) ? $p['desc_'.$PortableSPK->language] : $p['desc']; 
	$p['dname'] = isset($p['dname_'.$PortableSPK->language]) ? $p['dname_'.$PortableSPK->language] : $p['dname'];
	printf('<table>
<tr><td class="i" rowspan="3"><img src="data:image/png;base64,%s"/></td>
    <th><a href="%s">%s</a> <span>%s</span> %s</th>
    <td class="r"><span>%s</span></td></tr>
<tr><td class="sum">%s</td><td class="r"><span>%s</span></td></tr>
<tr><td colspan="2"><br/>%s</td></tr></table>',
		$p['icon'],str_replace('/./','/',htmlspecialchars($p['link'])), htmlspecialchars($p['dname']),
		htmlspecialchars($p['version']), ((isset($p['beta']) && $p['beta']) ? '<sup>beta</sup>':''), nice_size($p['size']),
		$p['md5'], implode(', ',$p['arch']), htmlspecialchars($p['desc']) );
}
echo("</div></body>\n</html>\n");
